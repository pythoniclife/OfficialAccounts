...
def time_bar(bar_char='■', time_second=10, prompt_info='注意！任务完成，{:0>2d}秒后自动退出！'):
    bar = bar_char * time_second

    for second in range(time_second):
        print('\r' + prompt_info.format(len(bar) - 1 - second) + bar[:len(bar) - 1 - second], end='')
        time.sleep(1)
    exit()
...
progress_bar = tqdm(file_list_pdf)
